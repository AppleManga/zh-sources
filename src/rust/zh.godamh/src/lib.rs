...
fn get_page_list(manga_id: String, chapter_id: String) -> Result<Vec<Page>> {
	...
	let line = images.get("line").as_int()?;
	let image_base_url = if line == 2 {
		format!("https://f40-1-4.g-mh.online")
	} else {
		format!("https://t40-1-4.g-mh.online")
	};
	...
	for (index, item) in list.enumerate() {
		...
		let image_path = item.get("url").as_string()?.read();
